    ...
    for epoch in range(opts.init_epoch, opts.n_epochs):
        ...
        for idx, data in enumerate(train_loader):
            ...
            trg_cls = torch.randint(
                0, opts.char_categories, (input_image.size(0), 1)
            ).to(
                device
            )  # bs, 1
            ...
            nr_out = neural_rasterizer(trg_seq, trg_img)

            output_img = nr_out["gen_imgs"]
            rec_loss = nr_out["rec_loss"]
            vggcx_loss = vggcxlossfunc(nr_out["gen_imgs"], trg_img)
            loss = (
                opts.l1_loss_w * nr_out["rec_loss"]
                + opts.cx_loss_w * vggcx_loss["cx_loss"]
            )
            ...
            if opts.tboard:
                ...
                writer.add_scalar(
                    "Loss/img_l1_loss", opts.l1_loss_w * rec_loss.item(), batches_done
                )
                writer.add_scalar(
            ...
            if opts.sample_freq > 0 and batches_done % opts.sample_freq == 0:
                ...
                for i, one_gt_seq in enumerate(svg_target):
                    cur_svg_file = os.path.join(
                        sample_dir,
                        f"train_epoch_{epoch}_batch_{batches_done}_no_{i}_svg.svg",
                    )
                    if i == 0:
            ...
            if opts.val_freq > 0 and batches_done % opts.val_freq == 0:
                ...
                with torch.no_grad():
                    for val_data in val_loader:
                        ...
                        val_trg_cls = torch.randint(
                            0, opts.char_categories, (val_input_image.size(0), 1)
                        ).to(
                            device
                        )  # bs, 1
                        val_trg_img = util_funcs.select_imgs(
                            val_input_image, val_trg_cls, opts
                        )
                        val_trg_seq = util_funcs.select_seqs(
                            val_input_sequence, val_trg_cls, opts
                        )
                        val_trg_seq = val_trg_seq.squeeze(1)
                        val_trg_seq = val_trg_seq.transpose(
                            0, 1
                        )  # seqlen, bs ,feat_dim
                        val_trg_char = util_funcs.trgcls_to_onehot(
                            val_input_clss, val_trg_cls, opts
                        )
                        # run the image encoder-decoder
                        val_nr_out = neural_rasterizer(
                            val_trg_seq, val_trg_char, val_trg_img
                        )
                    ...
                    val_img_sample = torch.cat(
                        (val_trg_img.data, val_output_image.data), -2
                    )
                    val_save_file = os.path.join(
                        sample_dir, f"val_epoch_{epoch}_batch_{batches_done}.png"
                    )
                    ...
                    for i, one_gt_seq in enumerate(val_svg_target):
                        cur_svg_file = os.path.join(
                            sample_dir,
                            f"val_epoch_{epoch}_batch_{batches_done}_no_{i}_svg.svg",
                        )
                        if i == 0:
                            gt_svg = render(one_gt_seq.cpu().numpy())
                            with open(cur_svg_file, "a") as f:
...
def main():
    ...
    opts.experiment_name = opts.experiment_name + "_neural_raster"
