...
def _zoom_out(path_list, add_baseline=0.0, per=22):
    ...
    for command in path_list:
        ...
        for arg in command[1:]:
            if is_even:
                args.append(float(arg) - ((24.0 - per) / 24.0) * 64.0 / 4.0)
                is_even = False
            else:
                args.append(float(arg) - add_baseline)
                is_even = True
...
def is_valid_path(pathunibfp):
    return pathunibfp[0] and len(pathunibfp[0]) <= MAX_PATH_COMMANDS
...
def create_example(pathunibfp, img_size):
    ...
    final["sequence"] = np.concatenate(
        (vector, np.zeros(((MAX_PATH_COMMANDS - final["seq_len"]), 10))), 0
    )
    ...
    # )
    final["sequence"] = (
        np.reshape(final["sequence"], [(MAX_PATH_COMMANDS + 1) * 10])
        .astype(np.float32)
        .tolist()
    )
