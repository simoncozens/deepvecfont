...
def train_main_model(opts):
    ...
    img_decoder = ImageDecoder(
        img_size=opts.image_size,
        input_nc=opts.bottleneck_bits + opts.char_categories,
        output_nc=1,
        ngf=16,
        norm_layer=nn.LayerNorm,
    )
    ...
    modality_fusion = ModalityFusion(
        img_feat_dim=16 * opts.image_size,
    ...
    svg_encoder = SVGLSTMEncoder(
        ...
        ff_dropout=opts.ff_dropout,
        rec_dropout=opts.rec_dropout,
    )

    svg_decoder = SVGLSTMDecoder(
        char_categories=opts.char_categories,
        bottleneck_bits=opts.bottleneck_bits,
        mode=opts.mode,
        max_sequence_length=(MAX_PATH_COMMANDS + 1),
    ...
    mdn_top_layer = SVGMDNTop(
        ...
        gauss_temperature=opts.gauss_temperature,
        dont_reduce=opts.dont_reduce_loss,
    )
    ...
    neural_rasterizer = NeuralRasterizer(
        ...
        hidden_size=opts.hidden_size,
        num_hidden_layers=opts.num_hidden_layers,
        ff_dropout_p=opts.ff_dropout,
        rec_dropout_p=opts.rec_dropout,
        input_nc=2 * opts.hidden_size,
        output_nc=1,
        ngf=16,
        bottleneck_bits=opts.bottleneck_bits,
        norm_layer=nn.LayerNorm,
    ...
    neural_rasterizer_fpath = os.path.join(
        "./experiments/dvf_neural_raster/checkpoints/neural_raster_"
    ...
    all_parameters = (
        ...
        + list(modality_fusion.parameters())
        + list(svg_encoder.parameters())
        + list(svg_decoder.parameters())
        + list(mdn_top_layer.parameters())
    )
    optimizer = Adam(
        all_parameters,
        lr=opts.lr,
    ...
    for epoch in range(opts.init_epoch, opts.n_epochs):
        for idx, data in enumerate(train_loader):
            ...
            if opts.use_nr:
                loss = (
                    opts.l1_loss_w * img_decoder_out["img_l1loss"]
                    + opts.pt_c_loss_w * vggpt_loss["pt_c_loss"]
                    + opts.kl_beta * kl_loss
                    + opts.mdn_loss_w * svg_losses["mdn_loss"]
                    + opts.softmax_loss_w * svg_losses["softmax_xent_loss"]
                    + opts.l1_loss_w * synsvg_nr_out["rec_loss"]
                )
            else:
                loss = (
                    opts.l1_loss_w * img_decoder_out["img_l1loss"]
            ...
            mdn_loss, softmax_xent_loss = (
                svg_losses["mdn_loss"],
                svg_losses["softmax_xent_loss"],
            )
            ...
            if opts.sample_freq > 0 and batches_done % opts.sample_freq == 0:
                ...
                save_file = os.path.join(
                    sample_dir, f"train_epoch_{epoch}_batch_{batches_done}.png"
                )
            ...
            if opts.val_freq > 0 and batches_done % opts.val_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    if opts.tboard:
                        ...
                        writer.add_scalar(
                            "VAL/img_l1_loss", val_img_l1_loss, batches_done
                        )
                        writer.add_scalar(
                            "VAL/img_pt_loss", val_img_pt_loss, batches_done
                        )
                        writer.add_scalar(
                            "VAL/cmd_softmax_loss", val_cmd_softmax_loss, batches_done
                        )
                        writer.add_scalar(
                            "VAL/coord_mdn_loss", val_coord_mdn_loss, batches_done
                        )
                        writer.add_scalar(
                            "VAL/synsvg_nr_rec_loss",
                            val_synsvg_nr_rec_loss,
                            batches_done,
                        )
        ...
        if epoch % opts.ckpt_freq == 0:
            ...
            model_file_paths.append(
                os.path.join(ckpt_dir, f"{opts.model_name}_{epoch}.imgenc.pth")
            )
            model_file_paths.append(
                os.path.join(ckpt_dir, f"{opts.model_name}_{epoch}.imgdec.pth")
            )
            model_file_paths.append(
                os.path.join(ckpt_dir, f"{opts.model_name}_{epoch}.seqenc.pth")
            )
            model_file_paths.append(
                os.path.join(ckpt_dir, f"{opts.model_name}_{epoch}.seqdec.pth")
            )
            model_file_paths.append(
                os.path.join(ckpt_dir, f"{opts.model_name}_{epoch}.modalfuse.pth")
            )
            model_file_paths.append(
                os.path.join(ckpt_dir, f"{opts.model_name}_{epoch}.mdntl.pth")
            )

            if torch.cuda.is_available() and opts.multi_gpu:
                for idx in range(len(model_modules)):
                    torch.save(
                        model_modules[idx].module.state_dict(), model_file_paths[idx]
                    )
            else:
                for idx in range(len(model_modules)):
...
def network_forward(data, mean, std, opts, network_modules):
    ...
    if opts.ref_nshot == 1:
        ref_cls = torch.randint(
            0, opts.char_categories, (input_image.size(0), opts.ref_nshot)
        ).to(device)
    else:
        ref_cls_upper = torch.randint(
            0, opts.char_categories // 2, (input_image.size(0), opts.ref_nshot // 2)
        ).to(
            device
        )  # bs, 1
        ref_cls_lower = torch.randint(
            opts.char_categories // 2,
            opts.char_categories,
            (input_image.size(0), opts.ref_nshot - opts.ref_nshot // 2),
        ).to(
            device
        )  # bs, 1
    ...
    for ref_id in range(0, opts.ref_nshot):
        ref_cls_multihot = torch.logical_or(
            ref_cls_multihot,
            util_funcs.trgcls_to_onehot(
                input_clss, ref_cls[:, ref_id : ref_id + 1], opts
            ),
        )
    ...
    ref_cls_multihot = ref_cls_multihot.expand(
        input_image.size(0), opts.char_categories, opts.image_size, opts.image_size
    )
    ...
    se_hidden_ly = torch.zeros(
        ref_seq_cat.size(0), ref_seq_cat.size(1), opts.hidden_size
    ).to(device)
    se_cell_ly = torch.zeros(
        ref_seq_cat.size(0), ref_seq_cat.size(1), opts.hidden_size
    ).to(device)
