...
def test_main_model(opts):
    ...
    modality_fusion_fpath = os.path.join(
        ckpt_dir, f"{opts.model_name}_{epoch}.modalfuse.pth"
    )
    modality_fusion.load_state_dict(
        torch.load(modality_fusion_fpath, map_location=device)
    )
    ...
    with torch.no_grad():
        for test_idx, test_data in enumerate(test_loader):
            ...
            save_file_merge = os.path.join(
                savedir_idx,
                "imgs_" + str(opts.image_size),
                f"merge_" + str(opts.image_size) + ".png",
            )
            ...
            for char_idx in range(opts.char_categories):
                ...
                save_file = os.path.join(
                    savedir_idx,
                    "imgs_" + str(opts.image_size),
                    f"{char_idx:02d}_" + str(opts.image_size) + ".png",
                )
                # save_image(img_sample, save_file, nrow=8, normalize=True)
                save_image(img_sample, save_file, normalize=True)

                img_sample_hr = output_img_hr[char_idx, ...].data
                save_file_hr = os.path.join(
                    savedir_idx,
                    "imgs_" + str(opts.image_size_sr),
                    f"{char_idx:02d}_" + str(opts.image_size_sr) + ".png",
                )
            ...
            syn_svg_merge_f = open(
                os.path.join(os.path.join(savedir_idx, "svgs"), f"syn_merge.html"), "w"
            )
            for sample_id, sampled_svg in enumerate(sampled_svg_list):
                ...
                for i, one_seq in enumerate(svg_dec_out):
                    syn_svg_outfile = os.path.join(
                        os.path.join(savedir_idx, "svgs"),
                        f"syn_{i:02d}_{sample_id:02d}.svg",
                    )
            ...
            for i, one_gt_seq in enumerate(svg_target):
                gt_svg_outfile = os.path.join(
                    os.path.join(savedir_idx, "svgs"), f"gt_{i:02d}.svg"
                )
...
def network_forward(data, mean, std, opts, network_moudules):
    ...
    trg_img = util_funcs.select_imgs(
        input_image.repeat(opts.char_categories, 1, 1, 1), trg_cls, opts
    )
    ...
    trg_seq = util_funcs.select_seqs(
        input_sequence.repeat(opts.char_categories, 1, 1, 1), trg_cls, opts
    )
    trg_seq = trg_seq.squeeze(1)
    # the one-hot target char class
    trg_char = util_funcs.trgcls_to_onehot(
        input_clss.repeat(opts.char_categories, 1, 1), trg_cls, opts
    )
    ...
    for tst in range(opts.test_sample_times):
        ...
        outputs = torch.zeros(trg_seq.size(0), trg_seq.size(1), opts.hidden_size).to(
            device
        )
        ...
        sampled_svg = torch.zeros(
            trg_seq.size(0), trg_seq.size(1), opts.seq_feature_dim
        ).to(device)
        for t in range(0, trg_len):
            # self sample results
            if t == 0:
                inpt_self = torch.zeros(trg_seq.size(1), opts.seq_feature_dim).to(
                    device
                )
            else:
                inpt_self = sampled_svg[t - 1]
            decoder_output_self = svg_decoder(inpt_self, hidden_self, cell_self)
            output_self, hidden_self, cell_self = (
                decoder_output_self["output"],
                decoder_output_self["hidden"],
                decoder_output_self["cell"],
            )
        ...
        trg_seqlen = util_funcs.select_seqlens(
            input_seqlen.repeat(opts.char_categories, 1, 1), trg_cls, opts
        )
